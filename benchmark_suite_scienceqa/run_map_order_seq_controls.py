...
DEFAULT_VARIANTS = ["semantic_random", "random_partition"]
SUPPORTED_VARIANTS = ["semantic", "semantic_random", "random_partition"]
...
def _grade_parts(train_ds: Dataset, seed: int) -> List[Dataset]:
    parts: List[Dataset] = []
    for grade_num in range(SCIENCEQA_GRADE_MIN, SCIENCEQA_GRADE_MAX + 1):
...
def build_random_partition_slices(train_ds: Dataset, seed: int, num_slices: int) -> tuple[Dataset, Dict]:
    if num_slices <= 0:
    ...
    for sid, idxs in enumerate(np.array_split(np.arange(len(shuffled)), num_slices)):
        ds_part = shuffled.select(idxs.tolist())
        ds_part = _remove_then_add_column(ds_part, "slice_id", [sid] * len(ds_part))
        parts.append(ds_part)
        slice_sizes.append(len(ds_part))

    meta = {
        "variant": "random_partition",
        "num_slices": int(num_slices),
        "slice_sizes": slice_sizes,
        "notes": "Whole training set shuffled, then repartitioned into random equal-size slices.",
    }
    return concatenate_datasets(parts), meta


def save_slice_dataset(train_ds: Dataset, out_dir: Path, meta: Dict, overwrite: bool) -> None:
    if out_dir.exists():
        if not overwrite:
            return
...
def build_variant_slices(variant: str, train_ds: Dataset, seed: int, num_slices: int) -> tuple[Dataset, Dict]:
    if variant == "semantic":
        return build_semantic_slices(train_ds, seed)
    if variant == "semantic_random":
        return build_semantic_random_slices(train_ds, seed)
    if variant == "random_partition":
        return build_random_partition_slices(train_ds, seed, num_slices)
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Build and optionally run order-MAP Seq-LoRA control experiments. "
            "Supports randomizing grade-slice order or rebuilding random slice partitions."
        )
    )
    ...
    ap.add_argument(
        ...
        default=str(
            ROOT
            / "iid_qwen35_8b_scienceqa_lora_map_leftpad"
    ...
    ap.add_argument(
        "--slice_root",
        type=str,
        default=str(ROOT / "slice_data" / "map_order_seq_controls"),
    )
    ap.add_argument(
        "--log_root",
        type=str,
        default=str(ROOT / "logs" / "map_order_seq_controls"),
    )
    ...
    map_base_dir = Path(args.map_base_dir).resolve()
    ...
    for seed in seeds:
        ...
        map_dir = map_base_dir / f"seed_{seed}" / "map_step_2000"
        if not map_dir.is_dir():
            raise FileNotFoundError(f"Order-MAP adapter not found: {map_dir}")
