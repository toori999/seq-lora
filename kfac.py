...
def save_input_hook(
    ...
    def input_hook(_module: nn.Module, pos_args: tuple[t.Tensor]) -> None:
        ...
        if h.dim() == 3:
            ...
            raise RuntimeError(f"Unexpected activation dim: {tuple(h.shape)}")

        if has_bias:
            a = t.hstack((a, t.ones_like(a[:, :1])))

        assert a.dim() == 2  # [B, l_in]
        if a.size(-1) < lr_threshold or n_kfac is None:
            ...
            if module_name not in activations:
                ...
                activations[module_name] = activations[module_name][0] + A, False
        else:
            if module_name not in activations:
            ...
            A_lr = incremental_svd(activations[module_name][0], a, svd_dtype, n_kfac)
            activations[module_name] = A_lr, True
...
def save_output_grad_hook(
    ...
    def output_grad_hook(_module: nn.Module, _, out_pos_grad: tuple[Tensor]) -> None:
        ...
        if g.dim() == 3:
            ...
            raise RuntimeError(f"Unexpected grad dim: {tuple(g.shape)}")

        assert s.dim() == 2  # [B, l_out]
        if s.size(-1) < lr_threshold or n_kfac is None:
            S = (s[..., None] @ s[:, None]).sum(0)
            if module_name not in output_grads:
                output_grads[module_name] = S, False
            else:
                output_grads[module_name] = output_grads[module_name][0] + S, False
        else:
            if module_name not in output_grads:
            ...
            S_lr = incremental_svd(output_grads[module_name][0], s, svd_dtype, n_kfac)
            output_grads[module_name] = S_lr, True
