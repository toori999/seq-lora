...
def get_cnst_schedule_with_warmup(optimizer, num_warmup_steps, num_training_steps, last_epoch=-1):
    """ 
    trying an exponential scheduler 
    """
    ...
    def lr_lambda(current_step):
        if current_step < num_warmup_steps:
            ...
            if current_step < 0.7 * num_training_steps:
                return 1
            elif (current_step < 0.9 * num_training_steps) and (current_step >= 0.7 * num_training_steps):
                return 0.1
            elif (current_step < num_training_steps) and (current_step >= 0.8 * num_training_steps):
                return 0.01
            else:
                return 0.0
        # return max(
...
class WrapperBaseeceprf(PeftModel):
    """
    ...
    def evaluate(self, eval_loader):
        ...
        for step, batch in enumerate(eval_loader):
            with torch.no_grad() and torch.inference_mode():
                logits = self.forward_logits(batch, sample=True, n_samples=ns).detach() # self.eval_n_samples
                ...
                if torch.isnan(nll):
                    if self.accelerator.is_local_main_process:
                        print(str(ns)+'nll:', nll)
                        print(str(ns)+'probs:', probs)
                        print(str(ns)+'logits:', logits)
                        exit()
                nlls.update(nll)
        ...
        if self.accelerator.is_local_main_process:
            if self.wandb_logger is not None:
                self.wandb_logger.log({
                    'val_acc_'+str(ns): val_acc, 
                    'val_ece_'+str(ns): val_ece, 
                    'val_nll_'+str(ns): val_nll, 
                    'std_'+str(ns):std,
                    'val_brier_'+str(ns): val_brier,
                })
    ...
    def fit_evaluate(self):
        ...
        with StageTimer(f"FIT {str(self.args.modelwrapper).upper()} on {source_task}(train)"):
        ...
        if hasattr(self.args, 'bayes_eval_n_samples_final'):
            all_mets = [] 
            for ns in [self.args.bayes_eval_n_samples_final]:
                self.eval_n_samples = ns
                with StageTimer(f"INFER {str(self.args.modelwrapper).upper()} on {eval_tag}"):
                    val_acc, val_ece, val_nll, val_brier = self.evaluate(self.test_loader)#self.evaluate(self.test_loader)
                all_mets.append([val_acc, val_ece, val_nll])
                logging.info(f'val_acc{ns}: {val_acc}, val_ece{ns}: {val_ece}, val_nll{ns}: {val_nll}, val_brier{ns}: {val_brier}')
                print(f"\n[{eval_tag}][{str(self.args.modelwrapper).upper()}]")
                print(
                ...
                if self.accelerator.is_local_main_process:
                    if self.wandb_logger is not None:
                        self.wandb_logger.log({
                            'final_val_acc'+str(ns): val_acc,
                            'final_val_ece'+str(ns): val_ece,
                            'final_val_nll'+str(ns): val_nll,
                            'final_val_brier'+str(ns): val_brier,
                            'table_key'+str(ns): wandb.Table(columns = ['acc','ece','nll'],data=[[val_acc,val_ece,val_nll]])
                                            })
            if self.accelerator.is_local_main_process:
                if self.wandb_logger is not None:
                    self.wandb_logger.log({
                        'All Metrics (N=0,5,10)': wandb.Table(columns=['acc','ece','nll'], data=all_mets)
                    })
