...
class ClassificationDataset:
    ...
    def loader(
        ...
        **kwargs,
        ...
        if subset_size > 0:
            subset_size = (
                int(len(self.dset[split]) * subset_size)
                # len(self.dset[split])
                # if len(self.dset[split]) < subset_size
                # else subset_size
            )
            dset = self.dset[split].select(range(subset_size))   #.shuffle(seed=subset_seed).select(range(subset_size))
            vdset = self.dset[split].select(range(subset_size, len(self.dset[split])))
        else:
        ...
        if is_s2s:
            if subset_size < 0: 
                ...
                return self.s2s_loader(dset, *args, **kwargs), self.s2s_loader(vdset, *args, **kwargs)
            # return self.s2s_loader(dset, *args, **kwargs)
        else:
            if subset_size < 0:
                return self.clm_loader(dset, *args, **kwargs)
            else: 
                return self.clm_loader(dset, *args, **kwargs), self.clm_loader(vdset, *args, **kwargs) 
            # return self.clm_loader(dset, *args, **kwargs)
