...
def get_all_dataset_names():
    return [
        dataset.split(".")[0]
        for dataset in os.listdir(PROJECT_ROOT / "dataset")
...
def get_dataset(dataset_name, accelerator, args: Namespace):
    ...
    assert (
        dataset_name in datasets_set
    ), f"Dataset {dataset_name} not found. Available datasets: {datasets_set.keys()}"
...
def get_all_models_names():
    return [
        model.split(".")[0]
        for model in os.listdir(PROJECT_ROOT / "models")
...
def get_all_modelwrappers_names():
    return [
        modelwrappers.split(".")[0]
        for modelwrappers in os.listdir(PROJECT_ROOT / "modelwrappers")
