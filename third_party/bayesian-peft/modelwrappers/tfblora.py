...
class TFBLoRA(WrapperBase):
    ...
    def fit(self, anchor_loader, target_ratio=0.01, max_iters=10): 
        ...
        with torch.no_grad() and torch.inference_mode():
            ...
            total_anchor_batches = len(anchor_loader)
            ...
            for iter_idx in range(max_iters):
                ...
                print(
                    f"[TFB-FIT] binary-search iter {iter_idx + 1}/{max_iters}: "
                    f"beta={self.args.bayes_beta:.6f} range=({low:.6f}, {high:.6f})",
                    flush=True,
                )
                ...
                for i, batch in enumerate(anchor_loader): 
                    ...
                    self._maybe_log_progress(
                        stage=f"TFB fit iter {iter_idx + 1}/{max_iters}",
                        step_idx=i,
                        total_steps=total_anchor_batches,
                        extra=f"beta={self.args.bayes_beta:.6f}",
                    )
                ...
                print(f"[TFB-FIT] NLL Loss change ratio: {loss_change_ratio * 100}%", flush=True)
                print('=====================', flush=True)
    ...
    def fit_evaluate(self):
        ...
        anchor_split_name = str(getattr(dataset_obj, "anchor_split_name", "anchor"))
        ...
        with StageTimer(f"FIT TFB on {source_task}({anchor_split_name})"):
            if self.args.iter == 0:
                self._update_lora_layers(self.lora_layers, self.args.bayes_beta)
            else:
                all_ori_predicted_classes = []
                all_ori_probs = []
                total_anchor_batches = len(self.anchor_loader)
            ...
            if self.accelerator.is_local_main_process:
                fit_artifact_path = Path(save_folder) / "tfb_fit.json"
                fit_artifact_path.write_text(
                    json.dumps(
                        {
                            "optimal_bayes_beta": float(self.args.bayes_beta),
                            "threshold": float(self.args.th),
                            "search_iters": int(self.args.iter),
                            "anchor_n_samples": int(self.train_n_samples),
                        },
                        indent=2,
                    )
                    + "\n",
                ...
                print(f"[Save] TFB fit info -> {fit_artifact_path}")
