...
class Kron:
    ...
    def init_from_model(cls, model, device):
        ...
        for name,p in model.named_parameters():
            # print('init_H', name, p.shape, p.requires_grad)
    ...
    def __add__(self, other):
        ...
        if len(self.kfacs) != len(other.kfacs):
            raise ValueError(
                f'Cannot add Kron factors with different block counts: '
                f'{len(self.kfacs)} vs {len(other.kfacs)}.'
            )
        for idx, (Fi, Fj) in enumerate(zip(self.kfacs, other.kfacs)):
            if len(Fi) != len(Fj):
                raise ValueError(
                    f'Cannot add Kron block {idx} with different factor counts: '
                    f'{len(Fi)} vs {len(Fj)}.'
                )
            for fac_idx, (Hi, Hj) in enumerate(zip(Fi, Fj)):
                if Hi.shape != Hj.shape:
                    raise ValueError(
                        f'Cannot add Kron block {idx} factor {fac_idx} with mismatched shapes: '
                        f'{tuple(Hi.shape)} vs {tuple(Hj.shape)}.'
                    )
    ...
    def _bmm(self, W: torch.Tensor) -> torch.Tensor:
        ...
        for Fs in self.kfacs:
            if len(Fs) == 1:
                Q = Fs[0]
                p = len(Q)
                W_block = W[:, cur_p:cur_p+p]
                if W_block.shape[1] != p:
                    raise RuntimeError(
                        f'Kron block size mismatch: expected {p} parameters at offset {cur_p}, '
                        f'got {W_block.shape[1]} out of total {P}.'
                    )
                W_p = W_block.T
                SW.append((Q @ W_p).T)
                cur_p += p
                ...
                if W_block.shape[1] != p:
                    raise RuntimeError(
                        f'Kron block size mismatch: expected {p} parameters at offset {cur_p} '
                        f'for factor shape ({p_in}, {p_out}), got {W_block.shape[1]} out of '
                        f'total {P}.'
                    )
                W_p = W_block.reshape(B * K, p_in, p_out)
        ...
        if cur_p != P:
            raise RuntimeError(
                f'Kron block size mismatch: consumed {cur_p} parameters but input has {P}.'
            )
    ...
    def _bmm(self, W: torch.Tensor, exponent: float = -1) -> torch.Tensor:
        ...
        for ls, Qs, delta in zip(self.eigenvalues, self.eigenvectors, self.deltas):
            if len(ls) == 1:
                ...
                if W_block.shape[1] != p:
                    raise RuntimeError(
                        f'KronDecomposed block size mismatch: expected {p} parameters at '
                        f'offset {cur_p}, got {W_block.shape[1]} out of total {P}.'
                    )
                W_p = W_block.T
                ...
                if W_block.shape[1] != p:
                    raise RuntimeError(
                        f'KronDecomposed block size mismatch: expected {p} parameters at '
                        f'offset {cur_p} for factor shape ({p_in}, {p_out}), got '
                        f'{W_block.shape[1]} out of total {P}.'
                    )
                W_p = W_block.reshape(B * K, p_in, p_out)
        ...
        if cur_p != P:
            raise RuntimeError(
                f'KronDecomposed block size mismatch: consumed {cur_p} parameters but input has '
                f'{P}.'
            )
        SW = torch.cat(SW, dim=1).reshape(B, K, P)
        return SW
