            ...
            if g is not None:
                if len(input_shape) == 2:
                    if g.shape[0] > N:
                        if g.shape[0] % N != 0:
                            raise RuntimeError(
                                f"Per-example gradient shape mismatch for module {module.__class__.__name__}: "
                                f"leading dim {g.shape[0]} is not divisible by batch size {N}."
                            )
                        seq_steps = g.shape[0] // N
                        grads.append(g.reshape(N, seq_steps, -1).sum(-2))
                    else:
                        grads.append(g)
                else:
                    if g.shape[0] > N*L:
...
class AsdlInterface(CurvatureInterface):
    ...
    @staticmethod
    def _trainable_local_param_flags(module):
        local_params = dict(module.named_parameters(recurse=False))
        trainable_local = {
            name: param for name, param in local_params.items()
            if param.requires_grad
        }
        if not trainable_local:
            return False, False

        unsupported = [name for name in trainable_local if name not in {'weight', 'bias'}]
        if unsupported:
            raise ValueError(
                f'Unsupported trainable local parameters for Kron/diag extraction in '
                f'{module.__class__.__name__}: {unsupported}'
            )

        return ('weight' in trainable_local), ('bias' in trainable_local)
    ...
    def _get_kron_factors(self, M):
        ...
        for module in self.model.modules():
            ...
            has_weight, has_bias = self._trainable_local_param_flags(module)
    ...
    def diag(self, batch, **kwargs):
        ...
        for module in self.model.modules():
            ...
            diag_stats = getattr(stats, 'diag', None)
            if diag_stats is None:
                raise ValueError(f'Module {module} is missing diagonal Fisher statistics.')
            if has_weight:
                if getattr(diag_stats, 'weight', None) is None:
                    raise ValueError(f'Module {module} is missing trainable weight diagonal Fisher stats.')
                vec.append(diag_stats.weight.flatten())
            if has_bias:
                if getattr(diag_stats, 'bias', None) is None:
                    raise ValueError(f'Module {module} is missing trainable bias diagonal Fisher stats.')
                vec.append(diag_stats.bias.flatten())
