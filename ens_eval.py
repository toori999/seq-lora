...
def _parse_eval_tasks(spec: str, default_task: str) -> List[str]:
    ...
    for raw in spec.split(","):
        ...
        if task == "iid":
            ...
            expanded.extend(["mmlu-math", "mmlu-cs", "mmlu-law", "mmlu-health"])
        else:
...
def _peft_set_adapter(model: PeftModel, name: str):
...
def _load_base_and_adapters(task: str, adapter_dirs: List[str], amp_dtype: torch.dtype, device: torch.device):
    for d in adapter_dirs:
        if not os.path.isdir(d):
            raise RuntimeError(f"Adapter dir not found: {d}")

    peft_cfg = PeftConfig.from_pretrained(adapter_dirs[0])
    base_name = peft_cfg.base_model_name_or_path
    print(f"[Load] base_model = {base_name}")
    for d in adapter_dirs:
        print(f"[Load] adapter = {d}")
...
def eval_deep_ensemble_one_dataset(model, loader, device, amp_dtype, temp: float):
    model.eval()
    ...
    active0 = _get_active_adapter_name(model)
    ...
    for batch in loader:
        ...
        std_sum += float(logits.std(dim=1).mean().item()) * bsz
    ...
    _peft_set_adapter(model, active0)
    return {
...
def main():
    ap = argparse.ArgumentParser(description="Run deep ensemble evaluation on IID/OOD tasks.")
    ap.add_argument(
        "--task",
        type=str,
        required=True,
    ...
    ap.add_argument("--adapter_dirs", type=str, required=True, help="Comma-separated adapter dirs ordered by ensemble member")
    ...
    ap.add_argument("--temp_ensemble", type=float, default=TEMP_ENSEMBLE)
    ...
    adapter_dirs = [x.strip() for x in args.adapter_dirs.split(",") if x.strip()]
    if len(adapter_dirs) < 2:
        raise ValueError("--adapter_dirs must contain at least 2 comma-separated adapter dirs.")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    amp_dtype = torch.bfloat16 if (device.type == "cuda" and torch.cuda.is_bf16_supported()) else torch.float16
    print("Using device:", device, "amp_dtype:", amp_dtype)

    with _StageTimer(f"LOAD-STAGE DeepEns on {args.task}"):
        tokenizer, model, num_classes = _load_base_and_adapters(
            task=args.task,
    ...
    print("    DEEP ENS ONLY       ")
    print("========================")
    print(f"[Config] n_models={len(adapter_dirs)} TEMP={args.temp_ensemble}")
    ...
    for eval_task in eval_tasks:
        ...
        with _StageTimer(f"INFER DeepEns on {eval_task}({split_name})"):
            m = eval_deep_ensemble_one_dataset(model, loader, device, amp_dtype, args.temp_ensemble)
        print(f"\n[{eval_task}({split_name})][DEEP-ENS]")
        print(
            f"  NLL={m['nll']:.4f}  ACC={m['acc']*100:.2f}%  ECE={m['ece']*100:.2f}%"
            f"  Brier={m['brier']:.4f}  std={m['std']:.4f}  n_models={m['n_models']}"
        )
